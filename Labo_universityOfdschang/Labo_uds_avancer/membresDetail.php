...
<body>
   ...
   <div class="collab-detail">
     ...
     if (isset($_GET['detail']) AND !empty($_GET['detail'])) {
        ...
        $id = $_GET['detail'];
        $sql =" SELECT image,nom,description,date_format(date, '%Y/%m/%d') AS date FROM member WHERE  id_part = $id";
    ...
    <!-- je construis la reponse a injecter dans la vu -->
      ...
      <div class="detail-member-collab">
          ...
          <p class="collab-name">Nom : <?=$row['nom'];?></p>     
      	 	...
      	 	<p class="collab-duree">Membre Depuis : <?=$row['date'];?></p>
      	 	<button class="back-another"><a href="membres.php" class="collaback">Retour</a></button>
      	 </div>
      </div>
